...
from mar_prior.corr_prior import ChannelPriorMultiScale, GaussPrior
...
class FlowNet(nn.Module):
	def __init__(self, batch_size, image_shape, hidden_channels, K, L, coupling_type, actnorm_scale=1.0):
		...
		self.c_prior = GaussPrior(batch_size,3,8,8,L)
		#ChannelPriorMultiScale(batch_size,3,32,32,L,mog=False,dp_rate=0,num_layers=3,hidden_size=32)
	...
	def encode(self, z, logdet=0.0):
		for layer, shape in zip(self.layers, self.output_shapes):
			...
			if isinstance(layer, Split2dMsC):
				z1, z2 = z
				#print('is_split ',z1.shape, z2.shape)
				logdet = logdet + self.c_prior((z1, z2),layer.level,reverse=False)
				z = z1
		logdet = logdet + self.c_prior(z,reverse=False)	#self.L,
		return z, logdet

	def decode(self, z, eps_std=None):
		z = self.c_prior(z,reverse=True)#self.L,
