...
class ChannelPriorUniScale(nn.Module):
	...
	def likelihood(self, mean, logs, z):
		return -0.5 * (logs * 2. + ((z - mean) ** 2) / torch.exp(logs * 2.) + self.Log2PI)	
	...
	def get_likelihood(self,z):	
		...
		z2_dp = self.dropout_in(z2.clone())
...
class GaussPrior(nn.Module):
	...
	def __init__(self,batch_size,nc,height,width,level = 1):
		super().__init__()
		self.Log2PI = float(np.log(2 * np.pi))
		self.batch_size = batch_size
		self.height = height//(2**(level))
	...
	def get_likelihood(self, z):
		mean, logs = torch.zeros_like(z), torch.zeros_like(z)
		if z.is_cuda:
			mean, logs = mean.cuda(), logs.cuda()
		return -0.5 * (logs * 2. + ((z - mean) ** 2) / torch.exp(logs * 2.) + self.Log2PI)

	def get_sample(self,z1=None):
		mean = torch.zeros(self.batch_size,self.nc,self.height,self.width).cuda()
		logs = torch.zeros(self.batch_size,self.nc,self.height,self.width).cuda()
		return (torch.randn(mean.size()).cuda())*torch.exp(logs) + mean #*0.75


	def forward(self,z,reverse=False):	
		if not reverse:
			log_likelihood = self.get_likelihood(z)
			#print('log_likelihood ',log_likelihood.shape)
			log_likelihood = torch.sum(log_likelihood, dim=(1,2,3))
